const buildKey = function (scene, parent, props) {
  if (props.type === "white") {
    ...
    const key = BABYLON.Mesh.MergeMeshes(
      [bottom, top],
      true,
      false,
      null,
...
const createScene = async function (engine) {
  ...
  for (let register = 1; register <= 7; register++) {
    keyParams.forEach((key) => {
      buildKey(
        ...
        keyboard,
        Object.assign(
          { register: register, referencePositionX: referencePositionX },
          key
        )
      );
    });
  ...
  BABYLON.SceneLoader.ImportMesh(
    "frame",
    "https://docs.microsoft.com/windows/mixed-reality/develop/javascript/tutorials/babylonjs-webxr-piano/files",
    "pianoFrame.babylon",
    scene,
    function (meshes) {
